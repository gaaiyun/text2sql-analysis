...
class TestPromptsCompliance(unittest.TestCase):
    ...
    def test_all_prompts_have_collate_rule(self):
        ...
        遍历 prompts/ 下实际存在的 *.md 提示词文件（如 *_optimized.md），
        逐一断言其包含 COLLATE 规则与字符集指定；不写死可能不存在的具体文件名。
        若 prompts/ 下没有任何 .md 文件（如部分用户本机数据未随仓库提供），则跳过。
        """
        prompt_files = sorted(glob.glob(os.path.join(self.prompts_dir, "*.md")))
        if not prompt_files:
            self.skipTest(
                "prompts/ 下没有提示词 .md 文件（用户本机数据，未随仓库提供）"
            )
        ...
        for filepath in prompt_files:
            ...
            self.assertIn("COLLATE", content, f"{name} 缺少 COLLATE 规则")
            self.assertIn("utf8mb4_unicode_ci", content, f"{name} 缺少字符集指定")
    ...
    def test_all_prompts_have_single_select_rule(self):
        ...
        for scenario in scenarios:
            ...
            if os.path.exists(filepath):
                ...
                has_single_select = (
                    "单个 SELECT" in content or "single SELECT" in content.lower()
                )
...
class TestSchemaEssential(unittest.TestCase):
    ...
    def setUp(self):
        schema_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "schema_gaaiyun_essential.md"
        )
        if not os.path.exists(schema_path):
            self.skipTest("schema_gaaiyun_essential.md 为用户本机数据，未随仓库提供")
    ...
    def test_collate_documented(self):
        """测试 COLLATE 文档化"""
        self.assertIn(
            "COLLATE utf8mb4_unicode_ci", self.schema_content, "缺少 COLLATE 说明"
        )
...
class TestExportTools(unittest.TestCase):
    ...
    def test_excel_export_exists(self):
        ...
        export_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "scripts", "export_excel.py"
        )
    ...
    def test_word_export_exists(self):
        """测试 Word 导出脚本存在"""
        export_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "scripts", "export_word.py"
        )
    ...
    def test_web_search_exists(self):
        ...
        search_path = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "scripts", "web_search.py"
        )
